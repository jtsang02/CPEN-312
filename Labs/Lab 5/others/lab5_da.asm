...
forever:	
	...
	mov a, b ; make sure b is not larger than 3
 	anl a, #3 ; ^ 
	mov b, a ; ^
	ljmp main ; Go check for more input
...
no_add:
	cjne a, #1, no_sub ; Check that we are doing subtraction
	lcall xchg_xy
...
no_sub:
	cjne a, #2, no_mul ; Check that we are doing multiplication 
	lcall xchg_xy
...
no_mul:
	cjne a, #3, nothing ; Check that we are doing division
	lcall xchg_xy
...
no_equal:
	; get more numbers
	lcall ReadNumber
	jnc no_new_digit ; Indirect jump to 'main'
	lcall Shift_Digits
	lcall Display
	
no_new_digit:
	ljmp main ; 'main' is to far away, need to use ljmp
